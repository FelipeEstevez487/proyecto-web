document.addEventListener('DOMContentLoaded', function() {
    ...
    if (galeria) {
        ...
        const contenedores = Array.from(galeria.children);
        
        // Mezclar aleatoriamente
        const contenedoresMezclados = mezclarArray(contenedores);
        
        // Limpiar la galería
        galeria.innerHTML = '';
        
        // Agregar los contenedores en orden aleatorio
        contenedoresMezclados.forEach(contenedor => {
...
function generarRecomendaciones(srcActual) {
    ...
    const cantidad = esDesktop ? 2 : 0; // 4 en desktop, 0 en mobile (porque se oculta)
